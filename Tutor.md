...
В большинстве случаев байткоды создаются специально для конкретного языка чтобы извлечь наибольшую пользу из гибридного подхода. Но иногда байткодом уже существующего языка начинают пользоваться языки новы. Так байткодом и виртуальной машиной Java(JVM) пользуются языки Clojure и Scala.
```mermaid
flowchart TD
    java[Java]
...
Итого:
- `архитектура` (в данном контексте)
- `машинный код`
- `интерпритация`
...
- `интерпретатор`
- `байткод`
- `виртуальная машина` (в данном контексте)
- `транспиляция`
- `ассемблер`
- `ассемблирование`
